...
public class BarcodeInnerNumberServiceImpl implements BarcodeInnerNumberService {
    ...
    public BarcodeInnerNumberServiceImpl(BarcodeInnerNumberDao barcodeInnerNumberDao,
            BarcodeInnerNumberMapper barcodeInnerNumberMapper, PostcodePoolDao postcodePoolDao) {
    ...
    @Override
    @Transactional
    public BarcodeInnerNumberDto save(long postcodeId,
            BarcodeInnerNumberDto barcodeInnerNumberDto) {
        PostcodePool postcodePool = postcodePoolDao.getById(postcodeId);
        if (postcodePool == null) {
            log.debug(
                    "Can't add barcodeInnerNumberDto to postcodePool. PostCodePool {} doesn't exist",
                    postcodeId);
            return null;
        }
        BarcodeInnerNumber barcodeInnerNumber =
                barcodeInnerNumberMapper.toEntity(barcodeInnerNumberDto);
        BarcodeInnerNumber barcodeInnerNumberSaved = barcodeInnerNumberDao.save(barcodeInnerNumber);
        // TODO not to get, but set previously created list and check in db if previous values not
        // erased
        postcodePool.getBarcodeInnerNumbers().add(barcodeInnerNumberSaved);
        log.info("Adding barcodeInnerNumber {} to postcodePool {}", barcodeInnerNumber,
                postcodePool);
    ...
    public BarcodeInnerNumberDto update(long id, BarcodeInnerNumberDto barcodeInnerNumberDto) {
        ...
        try {
            ...
            log.error("Can't get properties from object to updatable object for barcodeInnerNumber",
                    e);
        }
        target.setId(id);
